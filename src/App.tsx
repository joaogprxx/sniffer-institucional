...
// Stagger indents — each row sits at a different horizontal position
// creating a slow sine-wave cadence as the eye travels down the list.
// The pattern is: flush → slight → mid → slight → flush, forming a V-shape.
const rowIndents = ['0px', '5%', '10%', '5%', '0px'];
...
function BentoSection({ navigate: _navigate }: { navigate: ReturnType<typeof useNavigate> }) {
  ...
  return (
    <section id="produtos" style={{ padding: '80px 0 96px' }}>
      <div className={shell}>

        {/* Section header — editorial, sparse */}
        <motion.div
        ...
        >
          ...
          {/* Pull quote — right aligned, contrasts the heavy headline */}
          <p style={{
            margin: 0,
            fontSize: '15px',
            color: muted,
        ...
        <div className="hidden md:block">
          {/* Top rule */}
          <div style={{ height: '1px', background: 'rgba(17,16,38,0.10)', marginBottom: '0' }} />

          {items.map((item, i) => {
            const isHov = hovered === i;
            const isDimmed = anyHovered && !isHov;
            ...
            return (
              <motion.div
                ...
                style={{
                  marginLeft: rowIndents[i],
                  paddingRight: rowIndents[i],
                  borderBottom: '1px solid rgba(17,16,38,0.08)',
                  cursor: 'default',
                }}
              >
                <motion.div
                ...
                >
                  {/* Large ordinal — the visual anchor of each row */}
                  <motion.span
                    animate={{
                      color: isHov ? green : 'rgba(51,45,89,0.12)',
                    }}
                  ...
                  {/* Center column — tag + title stacked */}
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '20px', flexWrap: 'wrap' }}>
                    {/* Tag — small, uppercase, sits beside title */}
                    <motion.span
                      animate={{ color: isHov ? teal : 'rgba(17,16,38,0.30)' }}
                      transition={{ duration: 0.2 }}
                      style={{
                        fontSize: '10px',
                        fontWeight: 800,
                    ...
                    {/* Title — display scale, the weight of the row */}
                    <motion.h3
                      animate={{
                        color: isHov ? '#111026' : 'rgba(17,16,38,0.72)',
                      }}
                      transition={{ duration: 0.2 }}
                      style={{
                        margin: 0,
                  ...
                  {/* Description — revealed on hover, slides in from right */}
                  <motion.p
                    animate={{
                      opacity: isHov ? 1 : 0,
                      x: isHov ? 0 : 16,
                  ...
                  {/* Green slash — the signature element. Appears on hover as a structural mark
                      between number and title, angled 20deg, like a typographic slash from
                      a magazine layout. Positioned absolutely over the gap. */}
                  <motion.span
                    animate={{
                      opacity: isHov ? 1 : 0,
                      scaleY: isHov ? 1 : 0.4,
            ...
            >
              ...
              <div style={{ marginBottom: '48px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '24px' }}>
                  <div style={{ width: '48px', height: '3px', background: green, borderRadius: '2px' }} />
                  <span style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.12em', color: green, fontFamily: "'Ferom', Inter, sans-serif", textTransform: 'uppercase' }}>Sobre a plataforma</span>
                </div>
                <h2 style={{ ...h2Style, marginBottom: '0', maxWidth: '18ch' }}>Quem é a Sniffer</h2>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
                ...
                >
                  <blockquote style={{ margin: '0 0 32px', padding: '0 0 0 24px', borderLeft: `3px solid ${green}`, fontSize: 'clamp(20px, 2.5vw, 28px)', fontWeight: 400, lineHeight: 1.5, color: navy, fontFamily: "'Buasley', cursive", maxWidth: '32ch' }}>
                    "confiança se constrói no bairro, de pessoa pra pessoa, olho no olho."
                  </blockquote>
                  <p style={{ margin: '0 0 24px', fontSize: '17px', color: muted, lineHeight: 1.85, fontFamily: "'Ferom', Inter, sans-serif" }}>
                  ...
                  <p style={{ margin: '0 0 24px', fontSize: '17px', color: muted, lineHeight: 1.85, fontFamily: "'Ferom', Inter, sans-serif" }}>
                    Enquanto outras plataformas vendem visibilidade baseada em quem paga mais, a Sniffer criou um ecossistema onde as pessoas recomendam de verdade — com nome, com rosto e com limite. Cada usuário tem apenas <strong style={{ color: navy, fontWeight: 700 }}>7 Xodós</strong>: espaços no perfil onde ele declara publicamente em quem confia.
                  </p>
                  <p style={{ margin: 0, fontSize: '17px', color: muted, lineHeight: 1.85, fontFamily: "'Ferom', Inter, sans-serif" }}>
                ...
                >
                  <p style={{ margin: '0 0 32px', fontSize: '17px', color: muted, lineHeight: 1.85, fontFamily: "'Ferom', Inter, sans-serif" }}>
                    Para você, dono de negócio, a Sniffer não é mais uma rede social pra você alimentar com conteúdo e torcer pra alguém ver. É uma <strong style={{ color: navy, fontWeight: 700 }}>plataforma completa de presença, atendimento e inteligência</strong> — tudo num lugar só.
                  </p>
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                    {[
                      ...
                      <motion.div
                        ...
                        style={{ padding: '20px', borderRadius: '16px', background: '#F2F2F2', border: '1px solid rgba(17,16,38,0.06)' }}
                      >
                        <span style={{ fontSize: '22px', display: 'block', marginBottom: '10px' }}>{item.icon}</span>
                        <span style={{ fontSize: '14px', fontWeight: 800, color: navy, display: 'block', marginBottom: '6px', fontFamily: "'Ferom', Inter, sans-serif" }}>{item.name}</span>
                        <span style={{ fontSize: '13px', color: muted, lineHeight: 1.55, fontFamily: "'Ferom', Inter, sans-serif" }}>{item.desc}</span>
                      </motion.div>
                    ))}
                  </div>
                  <p style={{ margin: '28px 0 0', fontSize: '15px', color: muted, lineHeight: 1.75, fontStyle: 'italic', fontFamily: "'Ferom', Inter, sans-serif" }}>
